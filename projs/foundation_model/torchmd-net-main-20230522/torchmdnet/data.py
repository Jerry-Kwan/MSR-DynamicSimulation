...
class DataModule(LightningDataModule):
    def __init__(self, hparams, dataset=None):
        ...
        # jk midified - env
        # cannot use save_hyperparameters, use vars(...) instead
    ...
    def setup(self, stage):
        if self.dataset is None:
            ...
            if self.hparams["dataset_arg"] is not None:
                dataset_arg = self.hparams["dataset_arg"]
                # jk modified - env
                # need to convert dataset_arg from str to dict using json
                if isinstance(dataset_arg, str):
                    import json
                    dataset_arg = json.loads(dataset_arg)
            self.dataset = getattr(datasets, self.hparams["dataset"])(
                self.hparams["dataset_root"], **dataset_arg
            )
