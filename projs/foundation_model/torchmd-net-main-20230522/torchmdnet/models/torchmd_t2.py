...
class TorchMD_T2(nn.Module):
    r"""The TorchMD Transformer architecture (modified by jk).
    ...
    def __init__(
        ...
        super(TorchMD_T2, self).__init__()
        ...
        self.external_attn_layers = nn.ModuleList()
        for _ in range(num_layers):
            ...
            external_attn_layer = nn.MultiheadAttention(hidden_channels, num_heads, dropout=0.1, batch_first=True)
            self.attention_layers.append(layer)
            self.external_attn_layers.append(external_attn_layer)
    ...
    def forward(
        ...
        max_num_atoms = torch.max(bin_count)
        filter_list = pad_sequence([torch.ones(x) for x in bin_count]).T.bool().flatten()
        ...
        for i, attn in enumerate(self.attention_layers):
            ...
            # query = torch.zeros(batch_size, max_num_atoms, self.hidden_channels,
            #                     requires_grad=True, device=x.device, dtype=x.dtype)
            query = torch.zeros(batch_size * max_num_atoms, self.hidden_channels,
                                device=x.device, dtype=x.dtype)
            query[filter_list] += x
            query = query.reshape(batch_size, max_num_atoms, -1)

            ex_attn_out = self.external_attn_layers[i](query, external_embed, external_embed,
                                                       key_padding_mask=~mask.bool(), need_weights=False)[0]
            x = x + ex_attn_out.reshape(-1, self.hidden_channels)[filter_list]
